...
internal static class Core
{
    ...
    public static void Initialize()
    {
        ...
        Core.StartCoroutine(ClanTreasuryLend.Loop());
        Core.StartCoroutine(DebugPeekService.Loop());
        Core.StartCoroutine(SettingsFlushLoop());
        DropTracker.SeedExisting();
        Core.StartCoroutine(DropTracker.DiscoverLoop());
        Core.StartCoroutine(ScoopService.AutoScoopLoop());
    ...
    static IEnumerator SettingsFlushLoop()
    {
        var wait = new WaitForSeconds(2f);
        while (true)
        {
            yield return wait;
            PlayerSettings.FlushSettings();
        }
    }
