...
namespace Satisvampory.Services
{
    ...
    {
        // Same 5x input desire as ProcessConveyors ("Always desire 5x the transferring").
        ...
        /// Territory set matches ProcessConveyors: GetLogisticsTerritoryIds(standingPlot).
        /// </summary>
        public List<string> DescribeProductConveyor(int standingTerritoryId, PrefabGUID product)
        {
            ...
            var hasKindredCap = Core.PlayerSettings.TryGetItemCap(platformID, product, out var kindredCap);
            ...
            {
                if (!TryDescribeStationRecipe(station, product, platformID, haveProduct, hasKindredCap, kindredCap,
        ...
        bool TryDescribeStationRecipe(Entity station, PrefabGUID product, ulong platformID, int haveProduct, bool hasKindredCap, int kindredCap,
            out bool recipeOn, out bool plotCap, out bool outputFull, out int? capNumber, out List<PrefabGUID> inputs, out Dictionary<PrefabGUID, bool> inputAtFeedCap, out Dictionary<PrefabGUID, int> stationHave)
        {
            ...
            foreach (var recipe in recipesBuffer)
            {
                ...
                if (hasKindredCap)
                {
                    if (haveProduct >= kindredCap)
                    {
                        anyEnabledCapped = true;
                        continue;
                    }
                    remainingOutputs = kindredCap - haveProduct;
                    recipeCapped = true;
                }
            ...
            var productKindredCapped = hasKindredCap && haveProduct >= kindredCap;
            plotCap = recipeOn && (productKindredCapped || (anyEnabledCapped && !anyEnabledUncapped));
            if (plotCap && hasKindredCap)
                capNumber = kindredCap;
        ...
        static HashSet<int> CollectReceiverGroups(Entity station)
        {
            ...
            if (string.IsNullOrEmpty(name) || Core.Stash?.ReceiverRegex == null)
                return groups;
            foreach (Match match in Core.Stash.ReceiverRegex.Matches(name.ToLower()))
            {
        ...
        void SnapshotStashItem(Entity stash, PrefabGUID item, out int have, out int stackableHave)
        {
            have = 0;
            stackableHave = 0;
            ...
            foreach (var attachedBuffer in buffer)
            {
                ...
                foreach (var entry in inventoryBuffer)
                {
                    ...
                    have += entry.Amount;
                    if (entry.ItemEntity.Equals(NetworkedEntity.Empty) || entry.ItemType.Equals(Item_Building_Siege_Golem_T02))
        ...
        /// </summary>
        public List<string> DescribeConveyorNeed(int standingTerritoryId, int topN = 10)
        {
            var lines = new List<string>();
            if (standingTerritoryId < 0)
            {
            ...
            foreach (var id in logisticsIds)
                plotCounts[id] = CountTerritoryItems(id);
            var islandCounts = CountTerritoryItems(logisticsIds);
