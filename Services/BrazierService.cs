...
internal sealed class BrazierService
{
        ...
        public BrazierService() { index = HeartBoundIndex.Scan(includeDisabled: true, ComponentType.ReadOnly(Il2CppType.Of<Bonfire>())); for (var plot = TerritoryService.MIN_TERRITORY_ID; plot <= TerritoryService.MAX_TERRITORY_ID; plot++) proxTouched[plot] = new HashSet<Entity>(); lights = new ProxLights(index, proxTouched); }
