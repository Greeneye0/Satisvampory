    ...
    internal class StashService
    {
        ...
        public IEnumerable<Entity> GetStashesOnTerritory(int territoryIndex)
        {
            ...
            foreach (var sharedInventory in sharedCastleInventory)
            {
                ...
                if (StashRouting.IsNoShareName(name)) continue;
        ...
        public IEnumerable<(int group, Entity station)> GetAllReceivingStashes(int territoryId)
        {
            foreach (var result in GetAllGroupStashes(receiverRegex, territoryId))
            {
                ...
                if (name.Contains(OVERFLOW_SUFFIX) || StashRouting.IsOverflowDestName(name))
                    continue;
                if (result.station.Has<Refinementstation>())
                    continue;
                yield return result;
            }
        }
        ...
        IEnumerable<(int group, Entity station)> GetAllGroupStashes(Regex groupRegex, int territoryId)
        {
            ...
            for(var i = 0; i < sharedCastleInventory.Length; i++)
            {
                ...
                var name = stash.Read<NameableInteractable>().Name.ToString().ToLower();
                if (StashRouting.IsNoShareName(name))
                    continue;
        ...
        public void StashCharacterInventory(Entity charEntity)
        {
            try
            {
                ...
                var territoryIndex = Core.TerritoryService.GetStandingTerritoryId(charEntity);
                var csOn = Core.TerritoryService.IsClanShareOn(user);
                var rrGlobal = Core.PlayerSettings.IsRrGlobalEnabled(user.PlatformId);
                var onPlot = territoryIndex != -1;

                // RR global only gates using RR/stash off-plot. On a plot, ClanShare
                // already treats clan plots as one dest set — do not AND rrglobal.
                if (!onPlot)
                {
                    if (!rrGlobal)
                    {
                        Utilities.SendSystemMessageToClient(Core.EntityManager, user, "Unable to stash outside territories! (RR global off)");
                        return;
                    }
                    if (!csOn || Core.TerritoryService.GetLogisticsTerritoryIdsForCharacter(charEntity).Count == 0)
                    {
                        Utilities.SendSystemMessageToClient(Core.EntityManager, user, "Unable to stash outside territories — no clan plots (need ClanShare).");
                        return;
                    }
                }
                else
                {
                    ...
                    if (csOn && Core.TerritoryService.GetLogisticsTerritoryIdsForCharacter(charEntity).Count == 0)
                    {
                        Utilities.SendSystemMessageToClient(Core.EntityManager, user, "Unable to stash -- no clan castles available (ClanShare on).");
                        return;
                    }
                }
                ...
                ulong depositOwnerId = 0;
                if (territoryIndex >= 0)
                    Core.TerritoryService.TryGetTerritoryOwnerPlatformId(territoryIndex, out depositOwnerId);
                else
                {
                    var offPlotIds = Core.TerritoryService.GetLogisticsTerritoryIdsForCharacter(charEntity);
                    if (offPlotIds.Count > 0)
                        Core.TerritoryService.TryGetTerritoryOwnerPlatformId(offPlotIds[0], out depositOwnerId);
                }
                foreach (var stash in (clanWide ? GetAllAlliedStashesOnTerritory(charEntity) : GetStashesOnTerritory(territoryIndex)))
                {
                    try
                    {
                        // 1.6.1.37: RR dest on standing plot, every floor, include CastleWorkstation.
                        // Do not restrict dest to treasury floor.
                        ...
                        if (StashRouting.IsNoShareName(name))
                        {
                            StashRouting.LogDestPick(StashRouting.SkipLabel(name), territoryIndex, default, name, "stash-filter");
                            continue;
                        }
                        if (StashRouting.IsSpecialName(name))
                            continue;
                ...
                var overflowIds = clanWide
                    ? Core.TerritoryService.GetLogisticsTerritoryIdsForCharacter(charEntity)
                    : (IReadOnlyList<int>)new[] { territoryIndex };
                var overflowStashes = overflowIds.Where(id => !TerritoryService.IsHeartRaided(Core.TerritoryService.GetCastleHeart(id))).SelectMany(GetAllOverflowStashes)
                    .OrderBy(s => StashRouting.IsSpecialName(StashRouting.RawName(s)) ? 1 : 0)
                    .ThenBy(s => territoryIndex >= 0 && Core.TerritoryService.GetTerritoryId(s) == territoryIndex ? 0 : 1)
                    .ToArray();
                for (int i = ACTION_BAR_SLOTS; i < inventoryBuffer.Length; i++)
                {
                    ...
                    {
                        var success = false;
                        var stashEntries = StashRouting.OrderDepositDests(destCandidates, item, depositOwnerId, territoryIndex);
                            ...
                            foreach (var stashEntry in stashEntries)
                            {
                                try
                                {
                                    ...
                                    for (int j = 0; j < stashInventoryBuffer.Length; j++)
                                    {
                                        ...
                                        var depRank = StashRouting.RankDeposit(stashEntry.stash, item, depositOwnerId, true);
                                        StashRouting.LogDestPickAlways(depRank.Label + " class=" + depRank.Class, territoryIndex, item, StashRouting.DestName(stashEntry.stash), "stash");
                                        var leftover = Core.PlayerSettings.GetPullReserve(depositOwnerId, item);
                                        var beltEnt = StashRouting.PredictNextBeltDest(stashEntry.stash, item);
                                        var belt = beltEnt == Entity.Null ? "stays" : StashRouting.RawName(beltEnt);
                                        DestDebugLog.Move("stash", territoryIndex, item, 1, Entity.Null, stashEntry.stash, depRank.Label + "/c" + depRank.Class, leftover, belt);
                        ...
                        if (stashEntries.Count > 0)
                        {
                            foreach (var stashEntry in stashEntries)
                            {
                                try
                                {
                                    ...
                                    var beltEnt = StashRouting.PredictNextBeltDest(stashEntry.stash, item);
                                    var belt = beltEnt == Entity.Null ? "stays" : StashRouting.RawName(beltEnt);
                                    DestDebugLog.Move("stash", territoryIndex, item, transferredAmount, Entity.Null, stashEntry.stash, depRank.Label + "/c" + depRank.Class, leftover, belt);
                        ...
                        if (itemEntry.Amount > 0)
                        {
                            ...
                            foreach (var overflowStash in overflowStashes)
                            {
                                try
                                {
                                    ...
                                    DestDebugLog.Move("stash", territoryIndex, item, transferredAmount, Entity.Null, overflowStash, "overflow", Core.PlayerSettings.GetPullReserve(depositOwnerId, item), "stays");
                                    itemEntry.Amount = addItemResponse.RemainingAmount;
                                    if (!addItemResponse.ItemsRemaining)
                                    {
                                        InventoryUtilitiesServer.ClearSlot(Core.EntityManager, inventory, i);
                ...
                if (!Core.PlayerSettings.IsSilentStashEnabled(user.PlatformId))
                {
                    foreach (var ((stash, item), amount) in amountStashed)
                    {
                        ...
                        StashRouting.LogBeltTo(stash, item, territoryIndex, "stash");
                    }
