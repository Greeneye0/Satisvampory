...
namespace Satisvampory.Services
{
    ...
    internal static class DebugPeekService
    {
        ...
        static string Dispatch(string op, int plot, int guid, string name, bool apply, int users = 30, string dest = "", int seconds = 0)
        {
            ...
            {
                case "help":
                    return "{\"ops\":[\"help\",\"players\",\"plots\",\"plot\",\"item\",\"covering\",\"upgrade\",\"settings\",\"dest\",\"sim\",\"fair\",\"occupy\",\"guest\",\"cover\",\"unstick\",\"need\",\"needraw\",\"gearraw\",\"selftest\",\"log\",\"perf\",\"logdump\",\"servants\",\"servantstash\",\"thrones\",\"gotothrone\",\"hunt\",\"huntsend\",\"hunttime\",\"revive\",\"rhmax\"],"
                ...
                case "gearraw":
                    if (plot < 0)
                        plot = FirstStandingPlot();
                    return GearDebug.Snapshot(plot);
                case "needraw":
                    if (plot < 0)
                        plot = FirstStandingPlot();
                    return NeedDebug.Snapshot(plot);
                case "need":
                    if (plot < 0)
                        plot = FirstStandingPlot();
