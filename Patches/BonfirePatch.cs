...
public static class BonfireSystem_ServerPatch
{
    ...
    {
        if (!Core.HasInitialized || !Core.PlayerSettings.IsSolarEnabled(0)) return;
        if (Core.ServerGameManager.DayNightCycle.TimeOfDay != TimeOfDay.Day)
            SetLit(__instance, lit: true);
    }
    ...
    static void SetLit(BonfireSystem_Server system, bool lit)
    {
        ...
        try
        {
            ...
            {
                var brazier = rows[i];
                if (!brazier.Has<BurnContainer>() || !brazier.Has<NameableInteractable>() || !brazier.Has<Bonfire>())
                    continue;
                if (!brazier.Read<BurnContainer>().Enabled) continue;
                if (brazier.Read<NameableInteractable>().Name.ToString().IndexOf("night", System.StringComparison.OrdinalIgnoreCase) < 0)
                    continue;
