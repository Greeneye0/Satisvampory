...
- **Hand-dropped input gets five minutes** (1.0.145). Material a player puts into a hopper by hand is not dumped for **300 s** (`StationInputGrace.GraceSeconds`), even when it is short of a craft or above the keep, so a player can stage a partial recipe without the belt snatching it back. Detection is by count: the tick remembers what it last saw in each hopper and every mod move (`InventoryMove.CopyStacks`) adjusts that memory, so a count that **rose** on its own came from a player. Stations only consume, so decreases never trigger it. Limits: a first sight of a hopper after boot seeds without flagging; a player add that is fully masked by station consumption in the same window is not detected; the belt still **feeds** the station normally during the grace, and after it expires the normal dump rule applies.
