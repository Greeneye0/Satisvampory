...
- **Shattered shards are the `shattered` group, not `weapons`** (1.0.107). A `Shattered` plate takes shards by group word; a `Weapons` plate never does. `Shattered Weapons` is **AND** (weapons is a type word): shattered group AND the game's Weapon flag, so it takes shards and nothing else (1.0.124; between 1.0.107 and 1.0.123 the AND failed because shards had left the weapons group, and the plate matched nothing).
- **AND-mode type words fall back to the ItemCategory flag** (1.0.124). When a plate has a type word (`weapon`, `armor`, `material`) alongside other words, that type word is satisfied by the item's game category flag even if the item is not in the matching built-in group. A type word **alone** never gets the fallback.
