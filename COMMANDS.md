...
| `.pull plank 50` | You; source plot or clan | Take items into your bags. **Does not honor reserve.** Drains the worst dest for that item first, the chest it would be stashed into last. Never castle hearts |
...
| `.s alias` / `.s alias add sw Silkworm` / `.s alias del sw` | Castle (owner) | Castle item aliases, island-wide under ClanShare: chest plates, `--exclusions`, `.fi`, `.pull` on any clan castle. Castle alias wins over a server alias. Server-wide aliases stay admin (`.sg alias`) |
