...
| `.s alias` / `.s alias add sw Silkworm` / `.s alias del sw` | Castle (owner) | Castle-scoped item aliases: chest plates, `--exclusions`, `.fi`, `.pull` on this island. Castle alias wins over a server alias. Server-wide aliases stay admin (`.sg alias`) |
